...
contract SilensProposalVoting is Ownable, ReentrancyGuard {
    ...
    uint256 public votingPeriod = 10 minutes; // demo purposes
    uint256 public quorumPercentage = 20; // 20% of governance voters
    ...
    function executeProposal(uint256 _proposalId) external nonReentrant {
        ...
        if (totalVotes >= quorum && proposal.forVotes > proposal.againstVotes) {
            ...
            if (proposal.proposalType == ProposalType.APPROVE) {
                modelRegistry.updateModelStatus(proposal.modelId, SilensModelRegistry.ModelStatus.APPROVED);
            } else if (proposal.proposalType == ProposalType.FLAG) {
                modelRegistry.updateModelStatus(proposal.modelId, SilensModelRegistry.ModelStatus.FLAGGED);
            } else if (proposal.proposalType == ProposalType.DELIST) {
                modelRegistry.updateModelStatus(proposal.modelId, SilensModelRegistry.ModelStatus.DELISTED);
            }
        } else {
